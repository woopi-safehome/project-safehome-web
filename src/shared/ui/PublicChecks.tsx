...
const CHECKS = [
  {
    title: "집주인 체납·보증사고·전세보증 가입",
    body: "HUG 안심전세 앱에서 집주인의 세금 체납, 전세보증 사고 이력, 시세와 보증 가입 가능 여부를 확인하세요.",
    source: "주택도시보증공사",
    href: "https://www.khug.or.kr/jeonse/web/s01/s010102.jsp",
  },
  {
    title: "먼저 전입한 세입자",
    body: "주민센터에서 전입세대확인서를 열람하면, 이 집에 주민등록된 세대와 전입일자를 알 수 있어요.",
    source: "정부24",
    href: "https://www.gov.kr/mw/AA020InfoCappView.do?CappBizCD=13100000305",
  },
  {
    title: "집주인의 미납 국세",
    body: "임대차 시작일까지는 집주인 동의 없이 세무서에서 미납국세를 열람할 수 있어요. 홈택스로 미리 신청해 두세요.",
    source: "국세청",
    href: "https://www.nts.go.kr/nts/na/ntt/selectNttInfo.do?nttSn=1325154&mi=2207",
  },
] as const;

export function PublicChecks({ title = "등기부로 알 수 없는 것도 확인하세요" }: { title?: string }) {
  return (
    <section aria-labelledby="public-checks-title" className="rounded-2xl border border-brand/20 bg-brand-soft/60 p-5 sm:p-6">
      <h2 id="public-checks-title" className="text-base font-bold">
        {title}
      </h2>
      <p className="mt-1 text-sm text-muted">
        아래는 등기부에 나오지 않아 SafeHome 이 판단하지 못하는 위험이에요. 공공 서비스에서 직접 확인할 수 있어요.
      </p>
        ...
        {CHECKS.map((c) => (
          <li key={c.href}>
            <a
              ...
              className="group flex h-full flex-col gap-1.5 rounded-xl border border-line bg-surface p-4 transition-colors hover:border-brand/50"
            >
